...
func (s *DBTestSuite) TestGetEvaluationRulesNamespace() {
	...
	assert.Equal(t, rule1.SegmentKey, evaluationRules[1].Segments[1].Key)
	assert.Equal(t, secondSegment.MatchType, evaluationRules[1].Segments[1].MatchType)
	assert.Len(t, evaluationRules[1].Segments[1].Constraints, 0)
}
...
func (s *DBTestSuite) TestGetEvaluationRollouts() {
	...
	assert.Equal(t, segment.Key, evaluationRollouts[1].GetSegment().Segments[0].Key)
